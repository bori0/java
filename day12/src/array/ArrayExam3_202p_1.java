...
public class ArrayExam3_202p_1 {
	// #1.scanner 객체 생성 - 입력받기 위해
	static Scanner scan = new Scanner(System.in);
	//static(공통의 영역)이 붙어야 공통으로 다 쓰겠다는 의미
...
//				switch(selectNo) { case 1: 실행; break;....}
				if(selectNo==1) {//1.학생 수
					...
					scores = new int[studentCount];//점수를 담을 배열공간 생성
									//[.length]를 쓸 수 없다. >> .length는 배열의 길이만!
					inputScore(scores);
//					inputScore(new int[studentCount]);//주소열을 담는다는 것은 같으나,
													  //점수리스트에서 다 0으로 나옴
													  //위의 형식으로 쓰는 것이 좋다
	...
	public static void inputScore(int[] scores) {//지역변수, 여기에서 사라짐
	...
	public static void scoreList(int[] scores) {
		for(int a=0; a<scores.length; a++) {
			//안내문구
			System.out.println((a+1)+"번 학생 점수 = " +scores[a]);
//			scores[a]=scan.nextInt();//입력받은 점수 배열에 담기
		}//end of for
	}//end of scoreList
